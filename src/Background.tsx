...
function BackgroundSphere({ data }: { data: SphereData }) {
	...
	useFrame((state) => {
		...
		meshRef.current.position.set(Math.cos(a), 0, -Math.sin(a)).multiplyScalar(data.radius).setY(data.posY);
		...
		const scalePulse = 1 + Math.sin(t * data.twinkleSpeed * 2 + data.twinklePhase) * 0.1;
...
function BackgroundSpheres({ count = 340 }: { count?: number }) {
    const [spheresData] = useState(() => {
        return Array.from({ length: count }, () => ({
            ...
            starColor: new THREE.Color(1, randomArbitrary(230, 255) / 255, randomArbitrary(180, 220) / 255),
...
export function Background() {
	return (
		<div className="player-container" style={{ width: "100vw", height: "100vh" }}>
			<Canvas
