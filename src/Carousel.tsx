	...
	return (
		<>
			<mesh ref={icosahedronRef} scale={0.5}>
				...
				<meshPhongMaterial color={0x860808} shininess={100} specular={0xaaaaaa} />
			</mesh>
...
function CarouselSegment({ index, angle, texture }: { index: number; angle: number; texture: THREE.Texture }) {
	const meshRef = useRef<THREE.Mesh>(null);
	const [hovered, setHovered] = useState(false);

	const geometry = new THREE.CylinderGeometry(radius, radius, CONFIG.cylinderHeight, 10, 1, true, 0, segAngle);
	...
	return (
		<>
			<mesh ref={meshRef} geometry={geometry} material={material} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)} userData={{ segmentIndex: index, className: ZODIAC_SIGNS[index] }} />
			<Text3D 
			...
			>
				{ZODIAC_SIGNS[index]}
				<meshStandardMaterial color={0xe9d491} metalness={0.8} roughness={0.5} />
			</Text3D>
		</>
	);
}
	...
	return (
		<group ref={groupRef}>
			{ZODIAC_SIGNS.map((_, i) => {
				...
				return <CarouselSegment key={i} index={i} angle={angle} texture={textures[i]} />;
			})}
		</group>
	);
}
...
function Scene() {
	return (
		<>
			...
			<CarouselGroup />
			<CenterGeometry />
			<OrbitControls enableDamping={false} minPolarAngle={Math.PI / 2 - CONFIG.tiltAngle} maxPolarAngle={Math.PI / 2 + CONFIG.tiltAngle} enablePan={false} />
		</>
	);
}
...
function Carousel() {
	return (
		<div className="carousel" style={{ width: "100vw", height: "100vh" }}>
			<Canvas
				camera={{ position: [0, 0.125, 16], fov: 12 }}
				gl={{
					antialias: true,
					alpha: true,
