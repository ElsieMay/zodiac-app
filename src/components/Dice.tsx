...
function Dice({ cameraPosition = 3 }: { cameraPosition?: number }) {
  ...
  useEffect(() => {
    ...
    const meshMid = new THREE.MeshPhongMaterial({
      color: 0x860808,
      shininess: 100,
      specular: 0xaaaaaa,
    });
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const points = [
        new THREE.Vector3(
          Math.cos(lineAngle) * innerRadius,
          Math.sin(lineAngle) * innerRadius,
          0.2,
        ),
        new THREE.Vector3(
          Math.cos(lineAngle) * outerRadius,
          Math.sin(lineAngle) * outerRadius,
          0.2,
        ),
      ];
    ...
    for (let i = 0; i < 12; i++) {
      ...
      sphere.position.set(
        Math.cos(circleAngle) * 0.69,
        Math.sin(circleAngle) * 0.69,
        0.1,
      );
      scene.add(sphere);
    }
